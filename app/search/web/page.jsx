...
async function Web({ searchParams }) {
  ...
  if (!result) {
    return (
      <div className="flex justify-center font-nothing flex-col items-center mt-52">
  ...
  return (
    <div className="max-w-4xl mx-auto  px-3 sm:pl-[5%] md:pl-[14%] lg:pl-52 text-white ">
      <p className="bg-gradient-to-r font-nothing from-neutral-500 to-pink-600 bg-clip-text text-transparent mt-10">
      ...
      {result &&
        result.map((each) => (
          <div
            className="mb-3  mt-3 font-nothing hover:bg-neutral-900/30 hover:p-3 p-3 rounded-lg border border-neutral-800/15"
            key={each.link}
          >
            <div className="flex font-nothing flex-col group">
              <Link href={each.link}>{each.formattedUrl}</Link>
              <Link
                className="bg-gradient-to-r font-nothing from-neutral-800 to-white bg-clip-text text-transparent truncate font-medium text-lg"
                href={each.link}
              >
                {each.title}
              </Link>
